    ...
    function define_CesiumHeatmap_webgl() {
        var CesiumHeatmap_webgl = {
			defaults: {
				...
				minCanvasSize: 700,//1920,           // minimum size (in pixels) for the heatmap canvas  700
				maxCanvasSize: 2000,//3000,          // maximum size (in pixels) for the heatmap canvas 2000
...
function CHInstance_webgl(c, bb, o) {
	...
	this._spacing = this._options.radius * CesiumHeatmap_webgl.defaults.spacingFactor;
	...
	//chkun 初始化时
	//this._rectangle_update = this._rectangle;
	this._rectangle_update_tmp = this._rectangle;
	this._rectangle_update_src = this._rectangle;
...
CHInstance_webgl.prototype.setData = function(min, max, data) {
	if (data && data.length > 0 && min !== null && min !== false && max !== null && max !== false) {
		...
		for (var i = 0; i < data.length; i++) {
			var gp = data[i];
			this._heatmap_WebGL.addPoint(gp.x,gp.y, this._options.radius, this._options.intensity);//100，0.5
...
var UpdateHeatmapRectangle = function(){
	...
	var cenlon = (lonmax_src+lonmin_src)/2;
	var cenlat = (latmax_src+latmin_src)/2;

	var canvas_width = gheatmap_instance._canvas.width;
	var canvas_height = gheatmap_instance._canvas.height;
	var factor_llp = (lonmax_src-lonmin_src)/canvas_width;

	var lonmin = Cesium.Math.toDegrees(gheatmap_instance._rectangle_update_tmp.west);
	var latmin = Cesium.Math.toDegrees(gheatmap_instance._rectangle_update_tmp.south);
	var lonmax = Cesium.Math.toDegrees(gheatmap_instance._rectangle_update_tmp.east);
	var latmax = Cesium.Math.toDegrees(gheatmap_instance._rectangle_update_tmp.north);
    
    var height_factor = 2000;
    var resolution_factor_zero = 0.0000001;
    var resolution_factor_height = factor_llp;
    var resolution_metre = (factor_llp-resolution_factor_zero)/height_factor;
    if(height<height_factor){
    	var curresolution_heatmap = factor_llp-(height_factor-height)*resolution_metre;
        ...
        //gheatmap_instance.AddPointsAndUpdate()
    }
...
CHInstance_webgl.prototype.updateLayer = function() {
	...
	if (CesiumHeatmap_webgl.defaults.useEntitiesIfAvailable && this._cesium.entities) {
...
/*
   clear heatmap
 */
CHInstance_webgl.prototype.clear = function(){
 ...
 */
/*CHInstance_webgl.prototype.AddPointsAndUpdate = function(){
	this.clear();
	//add and render
    //this.setData()
};*/
