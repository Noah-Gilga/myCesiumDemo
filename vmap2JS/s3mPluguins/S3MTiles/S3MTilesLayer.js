define([
    ...
    function S3MTilesLayer(options) {
        ...
        this._maximumMemoryUsage = 2048;//-1;//chkun
        this._totalMemoryUsageInBytes = 3072;
        this._readyPromise = Cesium.when.defer();
        this._fromeIserver = options.fromIserver;
    ...
    S3MTilesLayer.prototype.loadConfig = function(url) {
        ...
        Cesium.when(url)
            ...
            .then(function(config) {
                ...
                if(that._fromeIserver){
                    //'http://10.1.55.49:8090/iserver/services/3D-test/rest/realspace/datas/Config/data/path/';
                    var defUrl = that._baseResource._url;
                    var index = defUrl.lastIndexOf('\/');
                    that._baseResource._url = defUrl.substring(0,index) + "/data/path/";
                }
