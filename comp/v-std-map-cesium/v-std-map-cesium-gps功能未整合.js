                ...
                if(!layer.disable){
                    ...
                    if (layer.layerType == "point") {
                        ...
                        if(layer.sameIcon !=undefined){
                            if(!layer.sameIcon){
                                //不统一图标
                                datasource = this.loadPointLayerGeoJsonDataByAttribute(layer.layerName, layer.url, layer.imageUrl, layer.scale, 0, layer.show,layer.supportSearch,layer.labelShow, layer.labelField, tempLableFontSize, layer.displayField, layer.pointFieldAttributeList,layer.iconField, layer.near, layer.far);
                            }else{
                                //所有点统一图标
                                datasource = this.loadPointLayerGeoJsonData(layer.layerName, layer.url, layer.imageUrl, layer.scale, 0, layer.show,layer.supportSearch,layer.labelShow, layer.labelField, tempLableFontSize, layer.displayField, layer.near, layer.far);
                            }
                        }else{
                            //所有点统一图标
                            datasource = this.loadPointLayerGeoJsonData(layer.layerName, layer.url, layer.imageUrl, layer.scale, 0, layer.show,layer.supportSearch,layer.labelShow, layer.labelField, tempLableFontSize, layer.displayField, layer.near, layer.far);
                        ...
                        if(layer.sameLineColor){
                            ...
                            //根据配置属性不同颜色
                            datasource = this.loadPolylineLayerGeoJsonDataByAttribute(layer.layerName, layer.url, tempWidth, layer.show, layer.labelShow, layer.labelField, tempLableFontSize, null, layer.lineField, layer.lineFieldAttributeList, layer.near, layer.far);
                        }
                        ...
                        if((layer.fillColorType !='all')){
                            datasource = this.loadPolygonLayerGeoJsonDataByAttribute(layer.layerName, layer.url, layer.show, layer.outlineColor, layer.colorField, layer.fieldAttributeList,tempWidth,layer.labelShow, layer.labelField, tempLableFontSize, layer.near, layer.far);
                            continue;
                        }
                        datasource = this.loadPolygonLayerGeoJsonData(layer.layerName, layer.url, null, layer.show, layer.fillColor, layer.outlineColor,tempWidth,0,layer.labelShow, layer.labelField, tempLableFontSize, layer.near, layer.far);
                    }
                    if(datasource){
                        //设置图层的默认显示字段和扩展显示字段
        ...
        seajs.use([
          ...
          "/vmap2/js/PhotoshopMathFP.js",                      //ps相关文件
            ...
            try{
                ...
                this.$$("canvas").style.filter = "invert(100%) sepia(80%) saturate(200%) hue-rotate(165deg) brightness(100%)";
            }
            catch(error){
 ...
 */
vtron.comp.std.map.Cesium.prototype.loadPointLayerGeoJsonData = function(layerName, url, imageUrl, scale = 1, height = 10, bShowLayer = true, supportSearch = false,labelShow, labelSelectField, labelFontSize, displayField, near = 0.0, far = 100000.0) {
 ...
 */
vtron.comp.std.map.Cesium.prototype.loadPolylineLayerGeoJsonData = function(layerName, url, width=5, color='rgba(0,200,0,0.8)', glowPower = 0.02, distanceDisplay = 1000, bShowLayer = true,labelShow, labelSelectField, labelFontSize, styleEx=null, gradientColor=false,near=0.0, far = 100000.0) {
    if(this._debugTyle) console.log('load:'+layerName);
 ...
 *                 有点是：同个图层具有相同的显隐性，而不会出现应为相机高度不同而出现同个图层某些要素显示而另一些要素隐藏。
 */
vtron.comp.std.map.Cesium.prototype.loadPolylineLayerGeoJsonDataByAttribute = function(layerName, url, width=5, bShowLayer = true,labelShow, labelSelectField, labelFontSize, styleEx=null, selectField, attributeInfo,near = 0,far = 100000) {
    if(this._debugTyle) console.log('load:'+layerName);
 ...
 */
vtron.comp.std.map.Cesium.prototype.loadPolylineLayerGeoJsonDataForPrimitives = function(layerName, url, width=5, color='rgba(0,200,0,0.8)', glowPower = 0.02, distanceDisplay = 1000, bShowLayer = true,labelShow, labelSelectField, labelFontSize, styleEx=null, gradientColor=false,near=0.0, far = 100000.0) {
    if(this._debugTyle) console.log('load:'+layerName);
 ...
 *      1  拔高处理
 */
vtron.comp.std.map.Cesium.prototype.loadPolygonLayerGeoJsonData = function(layerName, url, imageUrl, bShowLayer = true,fillColor=this.polygonOutlineColor,outlineColor=this.polygonOutlineColor,lineWidth=this.polygonOutlineWidth,type=0,labelShow, labelSelectField, labelFontSize, near, far) {
    if(this._debugTyle) console.log('load:'+layerName);
    //log 信息
    var vertexCount = 0;
    var entitiesCollection = this.getDataSourceByName(layerName);
 ...
 */
vtron.comp.std.map.Cesium.prototype.loadPolygonLayerGeoJsonDataByAttribute = function(layerName, url, bShowLayer = true, outlineColor, selectField,attributeInfo,outlineWidth,labelShow,labelSelectField, labelFontSize, near = 0,far = 100000){
    if(this._debugTyle) console.log('load:'+layerName);
 ...
 * }
 */
vtron.comp.std.map.Cesium.prototype.addPointElement = function(layerName, dataInfo, imageUrl, scale = 1, height = 0, bShowLayer = true, supportSearch = false, isScaleByDistance = true, billboardType = 'fastBillboard') {
    if(!this._viewer) return;
    if(!layerName) layerName = 'tempLayer';
    if(billboardType == 'normal'){
        ...
        //billboardCollection加载
        var datasource = this.getPrimitiveCollectionByName(layerName);
        if(!datasource) {
            //supportSearch && this.searchLayerArray.push(layerName);
            datasource = new Cesium.BillboardCollection(layerName);
            this._viewer.scene.primitives.add(datasource);
            datasource.name = layerName;
            datasource.geoType = 'fastBillboard';
        ...
        dataInfo.forEach((data) => {
            if(data.x&&data.x!=0 && data.y&&data.y!=0){
                ...
                var imagePicked = data.imageUrlPicked;
                //add
                var billboardPrimitive = datasource.add({
                    id: data.id, 
                    imageId: layerName,
                    image: data.imageUrl||imageUrl,
                    position: position,
                ...
                billboardPrimitive.start_x = data.x;
                billboardPrimitive.start_y = data.y;

                billboardPrimitive.imageSrc = imagematerial;//imageUrl||data.imageUrl;//初始设置的图标
                billboardPrimitive.imagePicked = imagePicked;
                billboardPrimitive.show = (typeof(data.Visibility)=="boolean")?data.Visibility:bShowLayer;
                billboardPrimitive.layerName = layerName;
                billboardPrimitive.distance = null;
                billboardPrimitive.normalize = null;
                billboardPrimitive.vec = null;
                //entity.pathTrack = [data.x,data.y,0]; 
                billboardPrimitive.pathTrack = []; 
            }
        });
    }
...
_updateDynamicPoint = function(instance){
    for(let data in instance._dynameLayer_dataSource){
        ...
        if(datasource.animate == true){
            ...
            if(datasource.geoType != 'fastBillboard'){
                ...
                for(var i=0;i<datasource._billboards.length;i++)
                {
                    var entity = datasource.get(i);
                    if(entity && entity ) {
                        var startpoint={},endpoint={};
                        startpoint.x = entity.start_x;
                        startpoint.y = entity.start_y;
...
vtron.comp.std.map.Cesium.prototype.movePointsEx = function(layerName, dataInfo,animate=true) {
    ...
    var datasource = this.getDataSourceByName(layerName) || this.getPrimitiveCollectionByName('billGPS');
    if(!datasource || !dataInfo) return;
    datasource.animate = true;
    ...
    if(datasource.geoType != 'fastBillboard'){
        ...
        var billboards = datasource._billboards;
        for(var i = 0; i< dataInfo.length; i++){
            for(var j = 0; j< billboards.length; j++){
                if(dataInfo[i].id == billboards[j].id){
                    var position = Cesium.Cartesian3.fromDegrees(dataInfo[i].x, dataInfo[i].y);
                    if(billboards[j].animate==true){
                        //当点再运动过程中，接受到新的位置时，用当前的终点作为新的起点
                        billboards[j].start_x = billboards[j].end_x;
                        billboards[j].start_y = billboards[j].end_y;
                    }
                    billboards[j].end_x = dataInfo[i].x;
                    billboards[j].end_y = dataInfo[i].y;
                    billboards[j].interval = dataInfo[i].intervaltime||3000;//default value:3000ms
                    billboards[j].start_time = new Date().getTime();
                    billboards[j].animate = true;
                    billboards[j].normalize = null;
                    billboards[j].distance = null;
                    billboards[j].vec = null;
                    if(billboards[j].playRealTimeTrack){
                    //只有在启动实时gps播放才计算
                    billboards[j].pathTrack.push(dataInfo[i].x,dataInfo[i].y,0);
                    }
                }
            }
        }
...
vtron.comp.std.map.Cesium.prototype.getPrimitiveById = function(layerName, id) {
    ...
    if(primitiveCollection) {
        var primitives = primitiveCollection._primitives || primitiveCollection._billboards;
        primitives.forEach((item) => {
...
vtron.comp.std.map.Cesium.prototype.showElements = function(layerName, fids) {
    ...
    if(datasource && datasource.geoType=='simpleLine') {
        fids.forEach(id => {
    ...
    if(datasource){ 
        ...
        if(fids) {
            for(let id of fids) {
                if(primitives.geoType=='fastBillboard'){
                    for(var j = 0; j< primitives._billboards.length;j++){
                        if(id == primitives._billboards[j].id){
                            primitives._billboards[j].show = false;
                            continue;
                        }
                    }
                }else{
        ...
        if(fids&&fids.length) {
            if(primitives.geoType!='fastBillboard'){
                ...
                for(var i = 0; i<fids.length; i++){
                    for(var j = 0; j< primitives._billboards.length;j++){
                        if(fids[i] == primitives._billboards[j].id){
                            primitives._billboards[j].show = false;
                            continue;
                        }
                    }
            ...
            if(primitives.geoType!='fastBillboard'){
                ...
                primitives._billboards.forEach((billboard)=>{
