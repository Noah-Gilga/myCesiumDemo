...
vtron.comp.std.map.Cesium = Polymer({
    ...
    properties: {
         ...
         * 手绘缓冲区的轮廓线颜色
         */
        drawBufferOutlineColor: {
            type: String,
            value: 'rgba(150, 255, 0, 1)',
            reflectToAttribute: true
        },
        ...
        drawBufferColor: {
            type: String,
            value: 'rgba(150, 255, 0, 0.5)',
            reflectToAttribute: true
        },

        /**
         * 手绘缓冲区的轮廓宽度，单位像素，pc端是2，大屏端是10
         */
        drawBufferOutlineWidth: {
            type: Number,
            value: 2,
            reflectToAttribute: true
        },
        ...
        imageLineMaterial: {
            type: Object,
            value: {
                'useImageMaterial': true,                   //要使用图片做材质这里要为true
                'animate': false,                           //图片要动起来 这里要为true
                'imageUrl': '/vmap2/images/joinLine.png',   //图片素材的地址
                'imageHW': 6                                //图片的长宽像素比,跟整条线的贴图数量有关，具体贴图数量地图自己算
            },
            reflectToAttribute: true
        },
    ...
    init: function(config, callback) {
        ...
        if(config.map){
            ...
            this.polylineOutlineWidth = lineWidth || this.polylineOutlineWidth*scale;
            this.bufferRadius = lineBuffer || this.bufferRadius;
            this.drawBufferOutlineColor = mapConfig.lineColor || this.drawBufferOutlineColor;
            this.drawBufferColor = mapConfig.lineColor || this.drawBufferColor;
            this.drawBufferOutlineWidth = lineWidth || this.drawBufferOutlineWidth*scale;
            this.drawBufferRadius = lineBuffer || this.drawBufferRadius;
            this.polygonOutlineColor = mapConfig.lineColor || this.polygonOutlineColor;
            this.polygonOutlineWidth = lineWidth || this.polygonOutlineWidth*scale;
            this.guideLineWidth = guideLineWidth || this.guideLineWidth*scale;
            this.circleOutlineColor = mapConfig.lineColor || this.circleOutlineColor;
            this.circleOutlineWidth = lineWidth || this.circleOutlineWidth*scale; 
            this.penLineColor = penLineColor || this.penLineColor;
            this.penLineWidth = penLineWidth || this.penLineWidth *scale;
            this.foldLineColor = penLineColor || this.foldLineColor;
            this.foldLineWidth = penLineWidth || this.foldLineWidth*scale;
            //this.imageLineMaterial = penLineColor || this.foldLineColor;
            this.imageLineWidth = penLineWidth || this.imageLineWidth*scale;
        ...
        this._init(dom, ()=>{
            ...
            //如果要接口分批传
            if(config.map.divideLoad && Number(config.map.timeIntervals) >0 && Number(config.map.linksNum) >0){
                var i =0;
                this.divideLoad = setInterval(() => {
                    var tempArr = config.layers.slice(i, i+Number(config.map.linksNum));
                    i = i+ Number(config.map.linksNum);
                    if(i<config.layers.length){
                        for (var layer of tempArr) {
                            if(!layer.disable){
                                var datasource = null;
                                if (layer.layerType == "point") {
                                    var tempLableFontSize;
                                    if($("body").hasClass("theme-wall")){
                                        tempLableFontSize = layer.wallLabelFontSize;
                                    }else{
                                        tempLableFontSize = layer.pcLabelFontSize;
                                    }
                                    if(layer.sameIcon !=undefined){
                                        if(!layer.sameIcon){
                                            //不统一图标
                                            datasource = this.loadPointLayerGeoJsonDataByAttribute(layer.layerName, layer.url, layer.imageUrl, layer.scale, 0, layer.show,layer.supportSearch,layer.labelShow, layer.labelField, tempLableFontSize, layer.displayField, layer.pointFieldAttributeList,layer.iconField, layer.near, layer.far, layer.labelOffset);
                                        }else{
                                            //所有点统一图标
                                            if(layer.framesIcon && layer.framesIcon!=undefined){
                                                let imageUrl = layer.framesImageUrl || '/vmap2/images/frames.png';
                                                let framesImageDetail = {
                                                    useFramesImage : true,
                                                    framesImageCol : layer.framesImageColNum, framesImageRow : layer.framesImageRowNum,
                                                    width: layer.framesImageWidth, height:layer.framesImageHeight,
                                                }
                                                datasource = this.loadPointLayerGeoJsonData(layer.layerName, layer.url, imageUrl, layer.scale, 0, layer.show,layer.supportSearch,layer.labelShow, layer.labelField, tempLableFontSize, layer.displayField, layer.near, layer.far, framesImageDetail, layer.labelOffset);
                                            }else{
                                                datasource = this.loadPointLayerGeoJsonData(layer.layerName, layer.url, layer.imageUrl, layer.scale, 0, layer.show,layer.supportSearch,layer.labelShow, layer.labelField, tempLableFontSize, layer.displayField, layer.near, layer.far, null, layer.labelOffset);
                                            }
                                        }
                                    }else{
                                        //所有点统一图标
                                    ...
                                    var tempWidth, tempLableFontSize;
                                    if($("body").hasClass("theme-wall")){
                                    ...
                                    if(layer.sameLineColor){
                                        //所有线统一颜色
                                        if(layer.usePrimitiveLine == undefined || !layer.usePrimitiveLine){
                                            datasource = this.loadPolylineLayerGeoJsonData(layer.layerName, layer.url, tempWidth, layer.color, null, null, layer.show, layer.labelShow, layer.labelField, tempLableFontSize, null, layer.gradientColor, layer.near,layer.far, layer.labelOffset);
                                        }else{
                                            datasource = this.loadPolylineLayerGeoJsonDataForPrimitives(layer.layerName, layer.url, tempWidth, layer.color, null, parseFloat(layer.far), layer.show, layer.labelShow, layer.labelField, tempLableFontSize, null, layer.gradientColor, layer.near,layer.far, layer.sameLineColor);
                                        }
                                        
                                        //nanchang
                                        // if(layer.loadType=="GeoJsonDataSource"){
                                        //     datasource = this.loadPolylineLayerGeoJsonData(layer.layerName, layer.url, tempWidth, layer.color, null, null, layer.show, layer.labelShow, layer.labelField, tempLableFontSize, null, layer.gradientColor, layer.near,layer.far);
                                        // }else if(layer.loadType=="primitiveCollection"){
                                        //     //chkun：道路静态图层优化
                                        //     datasource = this.loadPolylineLayerGeoJsonDataForPrimitives(layer.layerName, layer.url, tempWidth, layer.color, null, parseFloat(layer.far), layer.show, layer.labelShow, layer.labelField, tempLableFontSize, null, layer.gradientColor, layer.near,layer.far);
                                        // }
                                    }else{
                                        //根据配置属性不同颜色
                                        if(layer.usePrimitiveLine == undefined || !layer.usePrimitiveLine){
                                            datasource = this.loadPolylineLayerGeoJsonDataByAttribute(layer.layerName, layer.url, tempWidth, layer.show, layer.labelShow, layer.labelField, tempLableFontSize, null, layer.lineField, layer.lineFieldAttributeList, layer.near, layer.far, layer.labelOffset);
                                        }else{
                                            datasource = this.loadPolylineLayerGeoJsonDataForPrimitives(layer.layerName, layer.url, tempWidth, layer.color, null, parseFloat(layer.far), layer.show, layer.labelShow, layer.labelField, tempLableFontSize, null, layer.gradientColor, layer.near,layer.far, layer.sameLineColor, layer.lineField, layer.lineFieldAttributeList);
                                        }
                                    }
                                    ...
                                    if((layer.fillColorType !='all')){
                                        datasource = this.loadPolygonLayerGeoJsonDataByAttribute(layer.layerName, layer.url, layer.show, layer.outlineColor, layer.colorField, layer.fieldAttributeList,tempWidth,layer.labelShow, layer.labelField, tempLableFontSize, layer.near, layer.far, layer.labelOffset);
                                        continue;
                                    }
                                    datasource = this.loadPolygonLayerGeoJsonData(layer.layerName, layer.url, null, layer.show, layer.fillColor, layer.outlineColor,tempWidth,0,layer.labelShow, layer.labelField, tempLableFontSize, layer.near, layer.far, layer.labelOffset);
                                }
                                if(datasource){
                                    //设置图层的默认显示字段和扩展显示字段
                        ...
                        clearInterval(this.divideLoad);
                        console.log("分批加载已完成");
                    }
                }, Number(config.map.timeIntervals)*1000);
            }else{
    ...
    _init: function(dom, callback) {
        ...
        seajs.use([
          ...
          "/vmap2/js/PolylineTrailLinkMaterialProperty.js",                      //ps相关文件
          "/vmap2/js/PolylineImageRepeatMaterialProperty.js",                      //ps相关文件
        ], ()=>{
            //调试标识 LLL
            ...
            var defaultImageryProvider = this._createImageryProvider({
                ...
                mapUrl: this.mapUrl,//"/localTiles/base/{z}/{x}/{y}.png",//this.mapUrl,
...
vtron.comp.std.map.Cesium.prototype._initEvent = function() {
    ...
    if(this.custom_Cursor==undefined){
        ...
        if(this.offsetWidth<4096&&this.offsetHeight<4096){
            ...
            this.custom_Cursor['draw-imageLine']=this.addCursorStyle("/vmap2/images/cursor/pc_cursor/draw-line.png",1,0,0,false);//绘笔
    ...
    //地图高度前后变化
    this._lastHeightLevel = undefined;
    ...
    this._mapLineImageMaterialLayer = new Map();//使用图片材质的线图层集合，在相机缩放时要重新设定repeat数
    ...
    this._viewer.screenSpaceEventHandler.setInputAction((event) => {
        ...
        switch(this.optState) {
            ...
            case 'draw-circle':
                ...
                if(this.fireEventTag){
                    this.fire('drawCircle', {'optState' :'draw-circle',
                        ...
                        'circleOutlineColor':  this.circleOutlineColor
                    });
                }
                break;
            case 'choose-polygon':
				...
				if(this.optVertices.length<=2){
                    ...
                    this.fire('choosePolygonEndEvent', {'state':'choose-end','optState': 'choosePolygon','optID':this.optID});
					var result = this.searchByTurf(this.searchLayerArray, this.optID, this.optState, this.optVertices, this.bufferRadius,0,{},false);
					if(result.searchResultisNull == false){
                        if(this.fireEventTag){
                            this.optVertices.splice(this.optVertices.length - 1, 1, this.optVertices[0]);
            ...
            case 'draw-buffer':
                ...
                }else if(this.optState == 'draw-buffer'){
                    //标志结束
                    this.selectingTag = false;
                    this.changeState(this.optState,this.fireEventTag);//this.optState就是继续这个功能，'move'就是每次画完都回到平移状态,是否fire 事件的状态也要继承
                    if(linePos.length > 1){
                        var holeNum = this.getEntityById(this.optState, lineID+'.buffer').holeNum;
                        var otherIDs = [lineID+'.buffer', lineID+'.buffer.outline'];
                        for(let i = 0;i<holeNum;i++){
                            otherIDs.push(lineID+'.buffer.outline'+i);
                        }
                        if(this.fireEventTag){
                            this.fire('drawBufferEvent', {'optState': this.optState, 
                                'optID': lineID,
                                'otherIDs':otherIDs,
                                'holeNum':holeNum,
                                'points': this.getEntityById(this.optState, lineID+'.buffer').polygon.hierarchy.getValue(),
                                'originLinePoints':this.getEntityById(this.optState, lineID).polyline.positions.getValue(),
                                'drawBufferOutlineWidth': this.drawBufferOutlineWidth, 
                                'bufferRadius': this.drawBufferRadius,
                                'drawBufferColor': this.drawBufferColor,
                                'drawBufferOutlineColor': this.drawBufferOutlineColor,
                                'coordinateSystem':'cartesian', //表示是3D坐标，而'ellipsoidal' 经纬度
                                'eventState':'drawBufferEndTag', //结束本次绘制线
                                'synchronization': true            //同步
                            });
                        }else{
                            this.fire('drawBufferEndEvent', {'optState': this.optState, 
                                'optID': lineID, 
                                'otherIDs':otherIDs,
                                'holeNum':holeNum,
                                'points': this.getEntityById(this.optState, lineID+'.buffer').polygon.hierarchy.getValue(), 
                ...
                }else if(this.optState == 'draw-imageLine'){
                    //标志结束
                    this.selectingTag = false;
                    this.changeState(this.optState,this.fireEventTag);//this.optState就是继续这个功能，'move'就是每次画完都回到平移状态,是否fire 事件的状态也要继承
                    if(this.fireEventTag){
                        this.fire('drawImageLineEvent', {'optState': this.optState, 
                            'optID': lineID, 
                            'points': linePos, 
                            'imageLineWidth': this.imageLineWidth, 
                            'imageLineMaterial': this.imageLineMaterial,
                            'coordinateSystem':'cartesian', //表示是3D坐标，而'ellipsoidal' 经纬度
                            'eventState':'drawImageLineEndTag', //结束本次绘制线
                            'synchronization': true            //同步
                        });
                    }else{
                        this.fire('drawImageLineEndEvent', {'optState': this.optState, 
                            'optID': lineID, 
                            'points': linePos, 
                            'imageLineWidth': this.imageLineWidth, 
                            'imageLineMaterial': this.imageLineMaterial,
                    ...
                    if(entity) {
                        if(typeof(entity)=="object"){
                            ...
                            if(entity.geoType == "polyline"){
                                ...
                                var allPoints = [];
                                entity.polyline.positions.getValue().length>0 && entity.polyline.positions.getValue().forEach((position)=>{
                                    allPoints.push({
                                        x: myThis.getCartographicFromCartesian(position.x, position.y, position.z).lon, 
                                        y: myThis.getCartographicFromCartesian(position.x, position.y, position.z).lat
                                    })
                                });
                                //普通线及其他特殊线共用的属性
                                var data = {
                                    ...
                                    rgb: entity.polyline.material.getValue().color.toCssColorString(),
                                    coordinateSystem: 'ellipsoidal',
                                }
                                //贴图线静态
                                if(entity.polyline.material.getType() == 'PolylineImageRepeat' || entity.polyline.material.getType() == 'Image'){
                                    data.imageLineWidth = entity.polyline.width.getValue();
                                    data.imageLineMaterial = {
                                        useImageMaterial: true,
                                        animate: false,
                                        imageUrl: entity.polyline.material.getValue().image,
                                        imageHW: entity.imageHW,
                                        color : entity.polyline.material.getValue().color
                                    }
                                }else if(entity.polyline.material.getType() == 'PolylineTrailLink'){
                                    data.imageLineMaterial = {
                                        useImageMaterial: true,
                                        animate: true,
                                        imageUrl: entity.polyline.material.getValue().image,
                                        time: entity.polyline.material.getValue().time,
                                        animateColor : entity.polyline.material.getValue().color.toCssColorString()
                                    }
                                }
                            ...
                            if(entity.geoType == "polygon"){
                                ...
                                //缓冲线的面
                                if(entity.id.split('.').length>0){
                                    var allPoints = [];
                                    entity.polygon.hierarchy.getValue().positions.length >0 && entity.polygon.hierarchy.getValue().positions.forEach((position)=>{
                                        allPoints.push({
                                            x: myThis.getCartographicFromCartesian(position.x, position.y, position.z).lon, 
                                            y: myThis.getCartographicFromCartesian(position.x, position.y, position.z).lat
                                        })
                                    });
                                    let outlineEntity = myThis.getEntityById(entity.entityCollection._owner._name, entity.id+'.outline');
                                    let originEntity = myThis.getEntityById(entity.entityCollection._owner._name, entity.id.split('.')[0]);
                                    var orginPoints = [];
                                    originEntity.polyline.positions.getValue().length >0 && originEntity.polyline.positions.getValue().forEach((position)=>{
                                        orginPoints.push({
                                            x: myThis.getCartographicFromCartesian(position.x, position.y, position.z).lon, 
                                            y: myThis.getCartographicFromCartesian(position.x, position.y, position.z).lat
                                        })
                                    });
                                    data.originLinePoints = orginPoints;
                                    data.otherIDs = entity.id+'.outline';
                                    data.holeNum = entity.holeNum == undefined? 0: entity.holeNum;
                                    data.points = allPoints;
                                    data.coordinateSystem = 'ellipsoidal';
                                    data.drawBufferOutlineWidth = outlineEntity.polyline.width.getValue();
                                    data.bufferRadius = originEntity.drawBufferRadius;
                                    data.drawBufferColor = entity.polygon.material.color.getValue().toCssColorString();
                                    data.drawBufferOutlineColor = outlineEntity.polyline.material.color.getValue().toCssColorString();
                            ...
                            if(entity.geoType == 'circle'){
                                var pickPos = myThis.getCartographicFromCartesian(position.x, position.y, position.z);
                                var properties = {};
                                if(entity.properties){
                                    properties= entity.properties.getValue(Cesium.JulianDate.now());
                                }
                                var outlineEntity = myThis.getEntityById('draw-circle',entity.id+'.outline');
                                var data = {
                                    id: entity.id,
                                    _id: "_" + entity.id,
                                    optID: entity.id,
                                    ...
                                    circleColor: entity._ellipse.material.getValue().color.toCssColorString(),
                                    outlineWidth: outlineEntity.polyline.width.getValue(),
                                    circleOutlineColor: outlineEntity.polyline.material.getValue().color.toCssColorString()
                                }
                                myThis.fire("circleDoubleClickEvent", data);
                                return;
                            }
                            if(entity.polygon){
                            ...
                            //pickObject.id是string证明是用geometry加载的
                            if(pickObject.primitive.geoType=='simpleLine'){
                                var linePos = myThis.getCartographicFromScreenCoord(position.x,position.y);
                                var data = {
                                    id: entity,
                                    _id: "_" + entity,
                                    ...
                                    y: linePos.lat,
                                    //info: entity.otherProperty,
                                    layerName: pickObject.primitive.name,
                                }
                                myThis.fire("lineDoubleClickEvent", data);
                                return;
                            }
        ...
        switch(this.optState) {
            ...
            case 'draw-buffer':
                    ...
                    if(this.fireEventTag){
                        if(this.optState == 'draw-line'){
                            ...
                            this.fire('drawBufferEvent', {'optState': this.optState, 
                                'optID': this.optID, 
                                'points': this.optVertices, 
                                'drawBufferRadius': this.drawBufferRadius,
                                'drawBufferOutlineWidth': this.drawBufferOutlineWidth, 
                                'drawBufferColor': this.drawBufferColor,
                                'drawBufferOutlineColor': this.drawBufferOutlineColor,
                                'coordinateSystem':'cartesian', //表示是3D坐标，而'ellipsoidal' 经纬度
                                'eventState':'drawBufferBeginTag' //开始绘制线
                            });
                        }else if(this.optState == 'draw-imageLine'){
                            this.fire('drawImageLineEvent', {'optState': this.optState, 
                                'optID': this.optID, 
                                ...
                                'imageLineMaterial': this.imageLineMaterial,
                                'coordinateSystem':'cartesian', //表示是3D坐标，而'ellipsoidal' 经纬度
                                'eventState':'drawImageLineBeginTag' //开始绘制线
                            });
                        }
                    }
                }
                ...
                if(this.optState == 'draw-line'){
                    ...
                    this.addLineElement(this.optState, this.optID, this.optVertices, this.drawBufferOutlineWidth, this.drawBufferColor,this.drawBufferOutlineColor,0,'cartesian','polyline',1,this.drawBufferRadius);
                }else if(this.optState == 'draw-imageLine'){
                     this.addLineElement(this.optState, this.optID, this.optVertices, this.imageLineWidth, '','',0,'cartesian','polyline',0,0, this.imageLineMaterial);
                }
            ...
            case 'choose-polygon':
                ...
                this.fire('choosePolygonStartEvent',{'state':'choose-start','optState': 'choosePolygon','optID':this.optID});
                this._viewer.scene.requestRender();
                break;
            case 'draw-polygon':
                var position = this._viewer.camera.pickEllipsoid(event.position);
                this.optVertices.push(position);
                this.addPolygonElement(this.optState, this.optID, this.optVertices, this.polygonOutlineWidth, this.polygonColor, this.polygonOutlineColor, 0, 'cartesian');
                this._viewer.scene.requestRender();
                break;
            case 'choose-sector':
                var position = this._viewer.camera.pickEllipsoid(event.position);
            ...
            case 'choose-circle':
                ...
                this.addCircleElement(this.optState, this.optID, position, 1, this.circleOutlineColor, outlineColor.toCssColorString(),this.circleOutlineWidth);
                this._viewer.scene.requestRender();
                break;
            case 'draw-circle':
                // 首先判断圈的ID是否存在
                ...
                if(circle) break;
                this.optVertices.push(position);
                this.addCircleElement(this.optState, this.optID, position, 1, this.circleColor, this.circleOutlineColor,this.circleOutlineWidth);
                this._viewer.scene.requestRender();
                break;
            // case 'draw':
            //     //自由绘笔
        ...
        if(event) {
            ...
            if(Cesium.defined(pickedObject)){
                ...
                if(polygonEntity && polygonEntity.polygon){
                    ...
                    var properties = {};
                    if(polygonEntity.properties){
                        properties= polygonEntity.properties.getValue(Cesium.JulianDate.now());
                    }
                    var polygonDetail = {
        ...
        switch(this.optState) {
                ...
                if(this.optVertices.length) {
                    if(position)
                    {
                        ...
                        if(this.optState == 'draw-line'){
                            ...
                            this.addLineElement(this.optState, this.optID, tempLineVertices, this.drawBufferOutlineWidth, this.drawBufferColor,this.drawBufferOutlineColor,0,'cartesian','polyline',1,this.drawBufferRadius);
                        }else if(this.optState == 'draw-imageLine'){
                            this.addLineElement(this.optState, this.optID, tempLineVertices, this.imageLineWidth, '', '', 0,'cartesian','polyline',0,0, this.imageLineMaterial);
                        }
                    }
                }
                break;
            ...
            case 'draw-polygon':
                ...
                if(this.optVertices.length) {
                    ...
                    if(position){
                        var tempPolygonVertices = this.optVertices.concat(position);
                        this.addPolygonElement(this.optState, this.optID, tempPolygonVertices, this.polygonOutlineWidth, this.polygonColor, this.polygonOutlineColor, 0, 'cartesian');
                    }
                }
                break;
            case 'choose-sector':
            ...
            case 'choose-circle':
                ...
                if(this.optVertices.length) {
                    ...
                    if(position){
                        ...
                        var outlineColor = Cesium.Color.fromCssColorString(this.circleOutlineColor);
                        outlineColor.alpha = 1.0;
                        this.addCircleElement(this.optState, this.optID, this.optVertices[0], radius,this.circleOutlineColor,outlineColor.toCssColorString(),this.circleOutlineWidth);
            ...
            case 'draw-circle':
                    ...
                    if(this.optVertices.length) {
                        ...
                        if(position){
                            var radius = Math.max(Cesium.Cartesian3.distance(this.optVertices[0], position), 0);
                            this.optRadius = radius;
                            this.addCircleElement(this.optState, this.optID, this.optVertices[0], radius,this.circleColor,this.circleOutlineColor,this.circleOutlineWidth);   
                        }
                    }  
                    break;
            case 'delete':
    ...
    //     }
    // });
    this._viewer.scene.camera.moveStart.addEventListener(()=>{
        //记录移动前相机高度
        this._lastHeightLevel = this.heightConvertToLevel(this._viewer.camera.positionCartographic.height);
    })

    //相机移动结束事件
    this._viewer.scene.camera.moveEnd.addEventListener(()=>{
        var instance = this;
        let heightLevel = this.heightConvertToLevel(this._viewer.camera.positionCartographic.height);
        if(this._mapLineImageMaterialLayer.size>0 && heightLevel >this._lastHeightLevel){
            this._mapLineImageMaterialLayer.forEach(function(value, key, map){
                let datasource = value;
                let entities = datasource.entities.values;
                for(let i = 0;i<entities.length;i++){
                    let entity = entities[i];
                    if(entity.polyline && entity.polyline.material.getType()=="Image"){
                        entity.repeat = new Cesium.Cartesian2(entity.repeat.x*2, 1.0);
                    }
                }
            })
        }else if(this._mapLineImageMaterialLayer.size>0 && heightLevel <this._lastHeightLevel){
            this._mapLineImageMaterialLayer.forEach(function(value, key, map){
                let datasource = value;
                let entities = datasource.entities.values;
                for(let i = 0;i<entities.length;i++){
                    let entity = entities[i];
                    if(entity.polyline && entity.polyline.material.getType()=="Image"){
                        entity.repeat = new Cesium.Cartesian2((entity.repeat.x/((instance._lastHeightLevel-heightLevel)*2))<2? 2: entity.repeat.x/2, 1.0);
                    }
                }
            })
        }
    ...
    this._viewer.clock.onTick.addEventListener((clock) => {
        if(this.pathLog.length>0 && clock.shouldAnimate && this.pathLog[this.pathLog.length-1].tag == 'end'){
            for(var i =0;i<this.pathLog.length;i++){
                if((this.pathLog[i].time.dayNumber == clock.currentTime.dayNumber) && (this.pathLog[i].time.secondsOfDay <= clock.currentTime.secondsOfDay)){
                    ...
                    if(this.pathLog[i].isend !=undefined && this.pathLog[i].isend){
                        detail.isend = this.psathLog[i].isend;
                        ...
                        if(this.pathInfo && this.pathInfo!={}){
                            var difSecond = Cesium.JulianDate.compare(clock.currentTime, this.pathInfo.startTime);
                            var realSecond = difSecond * (this.pathInfo.realPlayTime/this.pathInfo.playTime);
                            var julianTime = Cesium.JulianDate.addSeconds(this.pathInfo.realStartTime, realSecond, new Cesium.JulianDate());
                            var isoTime = Cesium.JulianDate.toIso8601(julianTime).split("T");
                            timeDetail.time= isoTime[0]+" "+isoTime[1];
                            if(this.pathInfo._lastLog == undefined){
                                this.pathInfo._lastLog = [].concat(this.pathLog);
                            }
                        }
                        timeDetail.id = this.pathLog[i].id;
                        timeDetail.position = {x:this.pathLog[i].x, y:this.pathLog[i].y};
                        timeDetail.iskey = true;
                        this.fire("pathTrackerTime", timeDetail);
                        this.fire(this.pathLog[i].event.eventName,detail);
                        this.pathLog.shift();
                        if(this.pathLog.length == 0){
                            this.pathLog = this.pathInfo._lastLog;
                            this.pathLog[this.pathLog.length - 1].tag = '';
                            this.pathInfo._lastLog = undefined;
                            break;
                        }
                    }
                }
            ...
            if(this.pathInfo && this.pathInfo!={}){
                ...
                var realSecond;
                if(difSecond!=0){
                    realSecond = difSecond * (this.pathInfo.realPlayTime/this.pathInfo.playTime);
                }else{
                    realSecond = this.pathInfo.realPlayTime;
                }
                var julianTime = Cesium.JulianDate.addSeconds(this.pathInfo.realStartTime, realSecond, new Cesium.JulianDate());
                var isoTime = Cesium.JulianDate.toIso8601(julianTime).split("T");
                detail = {
                    'time': isoTime[0]+" "+isoTime[1]
                }
                //获取某一时刻当前正在运动的要素的坐标位置
                if(this.pathLog.length>0 && this._pathTrackingLayer_dataSource.length>0){
                    this._pathTrackingLayer_dataSource.forEach((datasource)=>{
                        var tempCar3 = datasource.entities.getById(this.pathLog[0].id).position.getValue(clock.currentTime);
                        detail.position = {
                            'x': this.getCartographicFromCartesian(tempCar3.x, tempCar3.y, tempCar3.z).lon,
                            'y': this.getCartographicFromCartesian(tempCar3.x, tempCar3.y, tempCar3.z).lat
                        }
                        detail.id = this.pathLog[0].id;
                    })
                }
                ...
                this.fire("pathTrackerTime", detail);
            }
 ...
 * framesImageDetail= {
 *      useFramesImage = true,//启用序列帧
 *      framesImageCol = 8, framesImageRow = 8,序列帧图片的长和宽
 *      width:,height:,每个billboard的长和宽设置
 * }
 */
vtron.comp.std.map.Cesium.prototype.loadPointLayerGeoJsonData = function(layerName, url, imageUrl, scale = 1, height = 10, bShowLayer = true, supportSearch = false,labelShow, labelSelectField, labelFontSize, displayField, near = 0.0, far = 100000.0, framesImageDetail = null, labelOffset) {
    ...
    ds.load({
        ...
        success: (data) => {
            ...
            for(var i = 0, len = data.features.length; i < len; i++) {
                ...
                if(!coord || typeof(coord[0]) != 'number'||typeof(coord[1]) != 'number'){
                    ...
                    var noCoordEntity = entitiesCollection.entities.add({
                        ...
                        billboard:{
                            ...
                            //distanceDisplayCondition : new Cesium.DistanceDisplayCondition(near, far), ljyDistance
                        }
                    });
                    var info = {};
                    info.level = 1;//warn
                ...
                if(framesImageDetail && framesImageDetail.useFramesImage){
                    entity.useFrames = true;
                    entitiesCollection.useFrames = true;
                    if(!entitiesCollection.billboardColNum || !entitiesCollection.billboardRowNum){
                        entitiesCollection.billboardColNum =  framesImageDetail.framesImageCol;
                        entitiesCollection.billboardRowNum =  framesImageDetail.framesImageRow;
                    }
                    entity.billboardColNum = framesImageDetail.framesImageCol;
                    entity.billboardRowNum = framesImageDetail.framesImageRow;
                    entity.billboard.height = framesImageDetail.height;
                    entity.billboard.width = framesImageDetail.width;
                }
                if(labelShow){
                    // 注意这里..结束
                    entity.label={
                        ...
                        font:'bold '+ (labelFontSize || "50")+'px MicroSoft YaHei',//'bold 36px MicroSoft YaHei',//'normal 32px MicroSoft YaHei',
                        showBackground : false,
                        scale : 0.5,
                        pixelOffset: new Cesium.Cartesian2(0,-(labelOffset || 40)),
 ...
 * layerName：alert|gps|yiyuan, bShowLayer: 默认显示加载层
 */
vtron.comp.std.map.Cesium.prototype.loadPointLayerGeoJsonDataByAttribute = function(layerName, url, imageUrl, scale = 1, height = 10, bShowLayer = true, supportSearch = false, labelShow,labelSelectField, labelFontSize, displayField, dataList, selectField, near = 0.0, far = 100000.0) {
    if(this._debugTyle) console.log('load:'+layerName);
    supportSearch && this.searchLayerArray.push(layerName);
            ...
            for(var i = 0, len = data.features.length; i < len; i++) {
                ...
                if(labelShow){
                    ...
                    entity.label={
                        ...
                        color : Cesium.Color.fromCssColorString('#fff'),
                        font:'bold '+ ((labelFontSize || "50") || "50")+'px MicroSoft YaHei',//'bold 36px MicroSoft YaHei',//'normal 32px MicroSoft YaHei',
                        showBackground : false,
                        scale : 0.5,
                        pixelOffset: new Cesium.Cartesian2(0,-(labelOffset || 40)),
                        horizontalOrigin : Cesium.HorizontalOrigin.CENTER,
 ...
 */
vtron.comp.std.map.Cesium.prototype.loadPolylineLayerGeoJsonData = function(layerName, url, width=5, color='rgba(0,200,0,0.8)', glowPower = 0.02, distanceDisplay = 1000, bShowLayer = true,labelShow, labelSelectField, labelFontSize, styleEx=null, gradientColor=false,near=0.0, far = 100000.0, labelOffset) {
    if(this._debugTyle) console.log('load:'+layerName);
    ...
    if(styleEx==null){
        ...
        promise.then((data) => {
            ...
            for(var i = 0, len = data.entities.values.length; i < len; i++) {
                ...
                // var extraId = entity.properties.getValue(this._viewer.clock._currentTime).id;
                // if(extraId){
                //     entity._id += '-'+String(extraId);''
                // }
                //entity.show = bShowLayer;
                ...
                if(labelShow){
                    ...
                    entity.label={
                        text: String(entity.properties[labelSelectField]),
                        color : Cesium.Color.fromCssColorString('#fff'),
                        font:'bold '+ ((labelFontSize || "50") || "50")+'px MicroSoft YaHei',// YaHei',//'normal 32px MicroSoft YaHei',
                        showBackground : false,
                        scale : 1.0,
                        pixelOffset: new Cesium.Cartesian2(0,-(labelOffset || 40)),
                        horizontalOrigin : Cesium.HorizontalOrigin.CENTER,
 ...
 *                 有点是：同个图层具有相同的显隐性，而不会出现应为相机高度不同而出现同个图层某些要素显示而另一些要素隐藏。
 */
vtron.comp.std.map.Cesium.prototype.loadPolylineLayerGeoJsonDataByAttribute = function(layerName, url, width=5, bShowLayer = true,labelShow, labelSelectField, labelFontSize, styleEx=null, selectField, attributeInfo,near = 0,far = 100000, labelOffset) {
    if(this._debugTyle) console.log('load:'+layerName);
...
/**
 * 添加线图层
 * loadPolylineLayerGeoJsonData是将json文件转成entities，而 loadPolylineLayerGeoJsonDataForPrimitives转成Primitives
 * 目的是提高效率,可能部分参数无效，比如渐变色。。。
 */
vtron.comp.std.map.Cesium.prototype.loadPolylineLayerGeoJsonDataForPrimitives = function(layerName, url, width=5, color='rgba(0,200,0,0.8)', glowPower = 0.02, distanceDisplay = 1000, bShowLayer = true,labelShow, labelSelectField, labelFontSize, styleEx=null, gradientColor=false,near=0.0, far = 100000.0, sameLineColor = true, lineField, lineFieldAttributeList) {
    if(this._debugTyle) console.log('load:'+layerName);

    var mapInstance = this;
    var datasource = undefined;
    var selectField = lineField, lineColorAttributeList = lineFieldAttributeList;
    $.getJSON(url,function(fileData){//url对应的json是标准的geojson数据
        var dataInfo = [];

        var fdata = fileData.features;
        for (var i = 0; i < fdata.length; i++) {
            var lineFeature = fdata[i];
            var lineObj = {};
            lineObj.id = lineFeature.properties.id;
            if(!sameLineColor){
                for(var j = 0; j<lineColorAttributeList.length; j++){
                    if(lineFeature.properties[selectField] == lineColorAttributeList[j].field){
                        lineObj.color= lineColorAttributeList[j].color;
                        continue;
                    }
                }
            }else{
            ...
            lineObj.geo = [];
            if(lineFeature.geometry.coordinates!=undefined){
                for(var j=0;j<lineFeature.geometry.coordinates.length;j++){
                    var coor = lineFeature.geometry.coordinates[j];
                    lineObj.geo.push(coor[0],coor[1]);
                }
                lineObj.width = width;
                dataInfo.push(lineObj);
            }
        }
        mapInstance.addPolylineDataSource(layerName,dataInfo,true);
        //nanchang
        // if(this._layerDisplayCondition){
        //     this.setLayerDisplayCondition(layerName,near,far,bShowLayer);
        // }
    })
}
 ...
 *      1  拔高处理
 */
vtron.comp.std.map.Cesium.prototype.loadPolygonLayerGeoJsonData = function(layerName, url, imageUrl, bShowLayer = true,fillColor=this.polygonOutlineColor,outlineColor=this.polygonOutlineColor,lineWidth=this.polygonOutlineWidth,type=0,labelShow, labelSelectField, labelFontSize, near, far, labelOffset) {
    if(this._debugTyle) console.log('load:'+layerName);
    //log 信息
    var vertexCount = 0;
    var entitiesCollection = this.getDataSourceByName(layerName);
 ...
 */
vtron.comp.std.map.Cesium.prototype.loadPolygonLayerGeoJsonDataByAttribute = function(layerName, url, bShowLayer = true, outlineColor, selectField,attributeInfo,outlineWidth,labelShow,labelSelectField, labelFontSize, near = 0,far = 100000, labelOffset){
    if(this._debugTyle) console.log('load:'+layerName);
        ...
        for(var i = 0;i<entities.length;i++){
            ...
            for(var j = 0; j< attributeInfo.length; j++){
                if(entity.properties[selectField]._value == attributeInfo[j].field){
                    ...
                    if(entity.polygon.extrudedHeight.getValue()<1){
                        entity.polygon.extrudedHeight = 0;
                    }else{
                        var linePos = [];
                        entity.polygon.hierarchy._value.positions.forEach((data)=>{
                            linePos.push(this.getCartographicFromCartesian(data.x, data.y, data.z).lon, this.getCartographicFromCartesian(data.x, data.y, data.z).lat, Number(entity.polygon.extrudedHeight.getValue()));
                        });
                        var heightOulineEntity = tempDatasource.entities.add({
                            id: entity.id+'.heightOutline',
                            polyline: {
                    ...
                    if(outlineWidth>1){
                        var outlineEntity = tempDatasource.entities.add({
                            id: entity.id+'.outline',//entity.id+'.outline',//String(entity.properties.getValue(this._viewer.clock._currentTime).id)+'.outline',
                            polyline: {
                                positions: entity.polygon.hierarchy._value.positions,//Cesium.Cartesian3.fromDegreesArrayHeights([start.lon, start.lat, start.alt, stop.lon, stop.lat, stop.alt]),
                                width: outlineWidth,
                                //distanceDisplayCondition : new Cesium.DistanceDisplayCondition(near, far),
                ...
                if(data.imageType=='frames'){
                    entity = datasource.entities.add({
                        ...
                        billboard: {
                            ...
                            width : data.height,
                            verticalOrigin: Cesium.VerticalOrigin.BOTTOM,
                            //distanceDisplayCondition: new Cesium.DistanceDisplayCondition(0.0, 200000000)
                        }
                    }); 
                    entity.useFrames = true;
                    ...
                    entity = datasource.entities.add({
                        ...
                        billboard: {
                            ...
                            scale: this.imageScaleFactor * scale,
                            verticalOrigin: verticalOriginType,
                            //scaleByDistance : new Cesium.NearFarScalar(1000, 0.5, 100000, 0.1),
                            //distanceDisplayCondition: new Cesium.DistanceDisplayCondition(0.0, 200000000)
                        }
                    }); 
...
/**
 * 生成记录线段中不同段要使用的repeat数目的材质
 * dataInfo = {
 *      [
 *          'color': rgb代表颜色 a值代表贴图数的1/10，这个在着色器中会自己算
 *          'ptnNum' 对应uv(取值0~1)的位置
 *      ]
 * }
 */
function getRepeatColorRamp(instance, dataInfo){
    var c=$(instance.$$("#myCanvas")).get(0);
    if(c){
        //之后要设置display none
        ...
        for(let i = 0; i<dataInfo.length; i++){
            grd.addColorStop(dataInfo[i].ptnNum, dataInfo[i].color);
        }
        ctx.fillStyle = grd;
        ctx.fillRect(0,0,170,1500);
        return c;
    }else{
        return null;
    }
 ...
 * imageMaterialStyle:{
 *      useImageMaterial:线的材质用图片，默认不开启
 *      animate：动态图片材质，未实现
 *      time: 运动时间
 *      imageHW： 传入图片的长宽比
 *      imageUrl: 图片纹理url
 *      animateColor: 动态的颜色
 *      color: '静态颜色//用于无视图片颜色 改变实际颜色
 *      type: 静态材质类别
 * }
 */
vtron.comp.std.map.Cesium.prototype.addLineElement = function(layerName, id, vertices, width, color, outlineColor = this.polylineOutlineColor, height = 0,coordinateSystem='cartesian',lineType='polyline',bufferType = 0,bufferRadius = this.bufferRadius,imageMaterialStyle={}) {
    if(!this._viewer) return;
    var datasource = this.getDataSourceByName(layerName);
    if(!datasource) {
        datasource = new Cesium.CustomDataSource(layerName);
    ...
    if(entity) {
        ...
        if(bufferType==1){
            ...
            if(buffGeo.length > 1){
                ...
                //holeArr [[holeArr1],[holeArr2],...]
                var bufferArr = [], holeArr = [], lineHoleCart = [];
                bufferCoords[0].forEach((tempArr)=>{
                    bufferArr.push(tempArr[0], tempArr[1]);
                });
                for(let i = 1; i<bufferCoords.length; i++){
                    holeArr[i] = [], lineHoleCart[i-1] = [];
                    bufferCoords[i].forEach((tempArr)=>{
                        holeArr[i].push(tempArr[0], tempArr[1]);
                    });
                    lineHoleCart[i-1] = Cesium.Cartesian3.fromDegreesArray(holeArr[i]);
                }
                //转Cartesian
                var lineBufferCart = Cesium.Cartesian3.fromDegreesArray(bufferArr);
                //var lineHoleCart = Cesium.Cartesian3.fromDegreesArray(holeArr);
                ...
                if(outlineColor==''||toutlineColor.alpha==1){
                    ...
                    this.addPolygonElement(layerName, id+'.buffer', lineBufferCart, width || this.polylineOutlineWidth, bufferColor, bufferColor,0,'cartesian',lineHoleCart);
                }else{
                    //虽然缓冲区是面，但是还是由线选的颜色决定-即color
                    this.addPolygonElement(layerName, id+'.buffer', lineBufferCart, width || this.polylineOutlineWidth, color, outlineColor, 0, 'cartesian',lineHoleCart);
                }
                entity.drawBufferRadius = bufferRadius;
                entity.originLinePointsTurf = buffGeo;
            }
        }
        //polyline(普通线)
        //根据是否用图片做材质又分成两类
        if(imageMaterialStyle.useImageMaterial){
            var lineMaterial;
            if(imageMaterialStyle.animate){
                //纹理图片是动态，且图层也要是动态的
                datasource.animate = true;
                lineMaterial = new Cesium.PolylineTrailLinkMaterialProperty(Cesium.Color.fromCssColorString(imageMaterialStyle.animateColor || 'rgba(255,0,0,1)'), (imageMaterialStyle.time||2000));
                entity.polyline.material = lineMaterial;
            }else{
                //cesium自带的图片纹理材质repeat
                if(imageMaterialStyle.type=='default'){
                    lineMaterial = new Cesium.ImageMaterialProperty({
                        image: imageMaterialStyle.imageUrl || '/vmap2/images/joinLine.png',
                        transparent : true,
                        granularity : 0.1,
                        repeat: {},//new Cesium.Cartesian2(10.0,1.0)
                    });
                    entity.polyline.material = lineMaterial;
                    entity.polyline.material.repeat = new Cesium.CallbackProperty(() => {return entity.repeat}, false);
                    //初始纹理重复次数=初始线段当前在屏幕上的长度和传入图片的长宽比的十倍
                    var totalScreenDistance = 0;
                    for(let ptLength = 0;ptLength<entityPosition.length-1;ptLength++){
                        let screenPtA = Cesium.SceneTransforms.wgs84ToWindowCoordinates(this._viewer.scene, entityPosition[ptLength]);
                        let screenPtB = Cesium.SceneTransforms.wgs84ToWindowCoordinates(this._viewer.scene, entityPosition[ptLength+1]);
                        totalScreenDistance+=Math.pow(Math.pow(Math.abs(screenPtA.x-screenPtB.x),2)+Math.pow(Math.abs(screenPtA.y-screenPtB.y),2),0.5);
                    }
                    entity.repeat = new Cesium.Cartesian2(totalScreenDistance/(imageMaterialStyle.imageHW || 2)/width, 1.0);
                    if(!this._mapLineImageMaterialLayer.get(layerName)){
                        this._mapLineImageMaterialLayer.set(layerName, datasource);
                    }
                }else{
                    var repeatInfo = [];
                    //根据不同段生成不同纹理信息的
                    for(let ptLength = 0;ptLength<entityPosition.length-1;ptLength++){
                        let screenPtA = Cesium.SceneTransforms.wgs84ToWindowCoordinates(this._viewer.scene, entityPosition[ptLength]);
                        let screenPtB = Cesium.SceneTransforms.wgs84ToWindowCoordinates(this._viewer.scene, entityPosition[ptLength+1]);
                        let screenDistance = Math.pow(Math.pow(Math.abs(screenPtA.x-screenPtB.x),2)+Math.pow(Math.abs(screenPtA.y-screenPtB.y),2),0.5);
                        //判断重复贴图数目
                        var picCount = screenDistance/(imageMaterialStyle.imageHW || 2)/width;
                        if(picCount<=1){
                            repeatInfo.push({
                                color: "rgba(0,0,255,"+picCount/10.0+")",
                                ptnNum: (ptLength/(entityPosition.length-1))+0.00000000001,//在第几段
                            },{
                                color: "rgba(0,0,255,"+picCount/10.0+")",
                                ptnNum: (ptLength+1)/(entityPosition.length-1),//在第几段
                            })
                        }else{
                            repeatInfo.push({
                                color: "rgba(255,0,0,"+picCount/10.0+")",
                                ptnNum: (ptLength/(entityPosition.length-1))+0.00000000001,//在第几段
                            },{
                                color: "rgba(255,0,0,"+picCount/10.0+")",
                                ptnNum: (ptLength+1)/(entityPosition.length-1),//在第几段
                    ...
                    //生成记录贴图数目的纹理
                    var repeatPic = getRepeatColorRamp(this,repeatInfo);
                    if(repeatPic!=null){
                        lineMaterial = new Cesium.PolylineImageRepeatMaterialProperty({
                            color: Cesium.Color.fromCssColorString(imageMaterialStyle.color || 'rgba(0,0,255,1)'), //着色器那里只使用图片的颜色 所以这个暂时不生效
                            image:imageMaterialStyle.imageUrl || '/vmap2/images/joinLine.png', 
                            exImg:repeatPic,//默认有空 
        ...
        if(lineType=='corridor' || !lineType){
            ...
            //第一次创建线且该线是带缓冲面的线
            if(bufferType == 1 && entityPosition.length>1){
                var options = {units: 'meters'};
                //将entityPosition转成geo格式
                var buffGeo = [];
                entityPosition.forEach((vertex)=>{
                ...
                if(outlineColor==''||toutlineColor.alpha==1){
                    outlineColor = 'rgba('+toutlineColor.red*255+','+toutlineColor.green*255+','+toutlineColor.blue*255+','+1.0+')';
                    bufferColor = 'rgba('+toutlineColor.red*255+','+toutlineColor.green*255+','+toutlineColor.blue*255+','+0.4+')';
                    polylineBufferInfo = this.addPolygonElement(layerName, id+'.buffer', lineBufferCart, width || this.polylineOutlineWidth, bufferColor/*this.PolygonColor*/, bufferColor/*this.lineColor*/,0,'cartesian');
                }else{
                    //原来传进来的就是透明
                    polylineBufferInfo = this.addPolygonElement(layerName, id+'.buffer', lineBufferCart, width || this.polylineOutlineWidth, color, outlineColor,0,'cartesian');
                }
                //记录原始线缓冲半径的大小,中间线段默认隐藏
                ...
                if(imageMaterialStyle.useImageMaterial){
                    ...
                    if(imageMaterialStyle.animate){
                        ...
                        if(imageMaterialStyle.type=='default'){
                            lineMaterial = new Cesium.ImageMaterialProperty({
                                image: imageMaterialStyle.imageUrl || '/vmap2/images/joinLineTEST.png',
                                transparent : true,
                                granularity : 0.1,
                                repeat: {},//new Cesium.Cartesian2(10.0,1.0)
                            });
                            ...
                            entity.polyline.material.repeat = new Cesium.CallbackProperty(() => {return entity.repeat}, false);
                            //初始纹理重复次数=初始线段当前在屏幕上的总长度和传入图片的长宽比的十倍
                            //每个线段都要算屏幕长度
                            var totalScreenDistance = 0;
                            for(let ptLength = 0;ptLength<entityPosition.length-1;ptLength++){
                                let screenPtA = Cesium.SceneTransforms.wgs84ToWindowCoordinates(this._viewer.scene, entityPosition[ptLength]);
                            ...
                            entity.repeat = new Cesium.Cartesian2(totalScreenDistance/(imageMaterialStyle.imageHW || 2)/width, 1.0);
                            entity.imageHW = (imageMaterialStyle.imageHW || 2);
                            if(!this._mapLineImageMaterialLayer.get(layerName)){
                                this._mapLineImageMaterialLayer.set(layerName, datasource);
                            }
                        }else{
...
vtron.comp.std.map.Cesium.prototype.addPolygonElement = function(layerName, id, vertices, outlineWidth, color, outlineColor = 'rgba(255, 255, 0, 0)', height = 0,coordinateSystem='cartesian', hole=[]) {
    if(!this._viewer) return;
    var datasource = this.getDataSourceByName(layerName);
    if(!datasource) {
        datasource = new Cesium.CustomDataSource(layerName);
    ...
    if(coordinateSystem=='cartesian'||!coordinateSystem){
        ...
        if(hole.length>0){
            for(let i = 0;i<hole.length;i++){
                entityHole.push({
                    positions:Cesium.Cartesian3.fromDegrees(Arrayhole[i])
                });
            }
        }
        //为了让轮廓线也正常加
    ...
    if(entity) {
        var lastHoleNum = entity.hierarchy.holes.length;
        entity.hierarchy = {
            positions: entityPosition,
            holes: entityHole
        };
        ...
        if(outlineWidth){
            ...
            //空洞outline处理
            if(hole.length>0){
                for(let i = 0;i<entityHole.length;i++){
                    this.addLineElement(layerName, id+'.outline'+i, entityHole[i].positions, outlineWidth, outlineColor, outlineColor, 0, coordinateSystem, lineType='polyline');
                }
                if(lastHoleNum > entityHole.length){
                    for(let j = lastHoleNum-entityHole.length;j<lastHoleNum;j++){
                        this.deleteElements(layerName, id+'.outline'+j);
                    }
                }
            }else{
                //hole.length == 0
                this.deleteElements(layerName, id+'.outline0');
            }
        }
        entity.holeNum = entityHole.length;
    }else {
        ...
        if(outlineWidth){
            ...
            if((outlineVertices.length>2)&&(outlineVertices[outlineVertices.length-1]!=outlineVertices[0])){
                outlineVertices.push(outlineVertices[0]);
            }
            if(outlineColor=='' || Cesium.Color.fromCssColorString(outlineColor).alpha!=1){
                var toutlineColor = Cesium.Color.fromCssColorString(outlineColor || color);
        ...
        if(typeof(vertices[0])=="number"){
            ...
            entity.hierarchy = new Cesium.PolygonHierarchy(Cesium.Cartesian3.fromDegreesArray(vertices), new Cesium.PolygonHierarchy(Cesium.Cartesian3.fromDegreesArray(holes)));
        }else{
            entity.hierarchy = new Cesium.PolygonHierarchy(vertices,new Cesium.PolygonHierarchy(hole));
        }
    ...
    if(useStyle && modelType=='White'){
        ...
        modelPrimitive.style = new Cesium.Cesium3DTileStyle({
            color: {
                ...
                //这样用需要生成3dtiles的时候保存好字段，要数值型的
                // conditions:[
                //     ["(${Floor} > 5)", "rgb(0, 123, 189, 1.0)"],
                //     ["(${Floor} > 0) && (${Floor} < 5)", "rgb(0, 255, 255,1.0)"],
                //     ["true", "color('rgb(0, 255, 255,1.0)')"]
                // ]
            }
        });
    }
        ...
        entities.forEach((entity)=>{
            if(entity&&entity.billboard) {
                if(imageUrl){
                    ...
                    entity.imageSrc = imageUrl;
                }
                if(data.imageUrlPicked){
                    entity.imagePicked = imageUrl;
                }
                entity.billboard.scale = this.imageScaleFactor*scale;
            }
        });
...
vtron.comp.std.map.Cesium.prototype.lineChangeStyle = function(layerName, dataInfo, width, color,linetype='polyline') {
    ...
    if(dataInfo.length > 0){
        if(datasource.geoType == 'simpleLine'){
            dataInfo.forEach((data) => {
                let lineColor = Cesium.Color.fromCssColorString(data.color || color);
                if(datasource.getGeometryInstanceAttributes(data.id)){
                    datasource.getGeometryInstanceAttributes(data.id).color = Cesium.ColorGeometryInstanceAttribute.toValue(lineColor);
                }
            });
        }else{
            dataInfo.forEach((data) => {
 ...
 *    {
 *       'id':id
 *        'drawBufferColor':'rgba()',//填充面颜色
 *        'drawBufferOutlineColor':'rgba()',//轮廓线颜色
 *        'drawBufferOutlineWidth': number//外轮廓线宽，单位像素
 *        'drawBufferRadius': number//缓冲区距离，地理距离，单位米
 *    }
 * ]
 */
vtron.comp.std.map.Cesium.prototype.bufferChangeStyle = function(layerName, dataInfo) {
    var datasource = this.getDataSourceByName(layerName);
    if(!datasource || !dataInfo) return;
    if(!(dataInfo instanceof Array)) dataInfo = [dataInfo];
    dataInfo.forEach((data) => {
        var entity = datasource.entities.getById(data.id);
        var bufferEntity = datasource.entities.getById(data.id + '.buffer');
        var bufferOutlineEntity = datasource.entities.getById(data.id + '.buffer.outline');
        var bufferHoleNum = bufferEntity.holeNum;
        if(entity) {
            //先判断是否要改填充面的颜色
            if(data.drawBufferColor && data.drawBufferColor!="" && !Cesium.Color.equals(Cesium.Color.fromCssColorString(data.drawBufferColor), bufferEntity.polygon.material.getValue().color)){
                bufferEntity.polygon.material = Cesium.Color.fromCssColorString(data.drawBufferColor);
            }
            if(data.drawBufferOutlineColor && data.drawBufferOutlineColor!="" && !Cesium.Color.equals(Cesium.Color.fromCssColorString(data.drawBufferOutlineColor), bufferOutlineEntity.polyline.material.getValue().color)){
                bufferOutlineEntity.polyline.material = Cesium.Color.fromCssColorString(data.drawBufferOutlineColor);
            }
            if(data.drawBufferOutlineWidth && data.drawBufferOutlineWidth!=bufferOutlineEntity.polyline.width.getValue()){
                bufferOutlineEntity.polyline.width = data.drawBufferOutlineWidth;
            }
            //如果缓冲距离变化就要重新计算缓冲面的坐标
            if(data.drawBufferRadius && data.drawBufferRadius!=entity.drawBufferRadius){
...
vtron.comp.std.map.Cesium.prototype.polygonChangeExtrudedHeight = function(layerName, dataInfo) {
    ...
    dataInfo.forEach((data) => {
        ...
        if(entity) {
            ...
            let linePos = [];
            entity.polygon.hierarchy.getValue().positions.forEach((pos)=>{
                linePos.push(this.getCartographicFromCartesian(pos.x, pos.y, pos.z).lon, this.getCartographicFromCartesian(pos.x, pos.y, pos.z).lat, data.height);
            });
            var outlineEntity = datasource.entities.getById(data.id+'.heightOuline');
            if(!outlineEntity){
                //增加水平方向上的线宽
                var outlineEntity = datasource.entities.add({
                    id: entity.id+'.heightOuline',
                    polyline:{
                        positions: Cesium.Cartesian3.fromDegreesArrayHeights(linePos),
                        width: data.width || 5,//this.getEntityById(layerName+'.outline', entity.id+'.outline').polyline.width.getValue(),
                        //distanceDisplayCondition : entity.polygon._distanceDisplayCondition.getValue(),
                        material: Cesium.Color.fromCssColorString(data.color || 'rgba(125,125,0,1)')//this.getEntityById(layerName+'.outline', entity.id+'.outline').polyline.material.getValue().color,
                    }
                });
            }else{
                outlineEntity.polyline.positions = Cesium.Cartesian3.fromDegreesArrayHeights(linePos);
            }
        }
    });
};
...
vtron.comp.std.map.Cesium.prototype.showElements = function(layerName, fids) {
    ...
    if(primitives && primitives.geoType=='simpleLine') {
        fids.forEach(id => {
            primitives.getGeometryInstanceAttributes(id).show = Cesium.ShowGeometryInstanceAttribute.toValue(true);
        });
        return;
    }
    if(datasource){ 
...
vtron.comp.std.map.Cesium.prototype.hideElements = function(layerName, fids) {
    ...
    if(primitives && primitives.geoType=='simpleLine') {
        fids.forEach(id => {
            primitives.getGeometryInstanceAttributes(id).show = Cesium.ShowGeometryInstanceAttribute.toValue(false);
        });
        return;
    }
    if(datasource){     
...
vtron.comp.std.map.Cesium.prototype.layerDelete = function(layerName, dataInfo, bPrimitive = false) {
    if(!layerName) {
        ['choose-line', 'draw-line', 'choose-polygon','choose-sector', 'choose-circle', 'draw', 'tempLayer','draw-buffer', 'draw-imageLine'].forEach((name) => {
    ...
    if(this._mapLineImageMaterialLayer.get(layerName)){
        this._mapLineImageMaterialLayer.delete(layerName);
    }
};
...
vtron.comp.std.map.Cesium.prototype.searchByTurf = function(layerNameArray, optID, optState, vertices, lineWidth, radius = 0, otherProperty,bCmd, stopFire) {
    ...
    switch(optState) {
        ...
        case 'choose-polygon':
            ...
            vertices.splice(vertices.length - 1, 1, vertices[0]);
            result.searchPoly.GeoPolyLineString = vertices;
            break;
        case 'choose-sector':
    ...
    layerNameArray.forEach((layerName) => {
        ...
        datasource && result.searchResult.push({
            Information: propertiesResult,
            LayerName: layerName,
            Layer:{
                useFrames: datasource.useFrames? 'frames': 'point',
                billboardColNum: Number(datasource.billboardColNum) || 1,
                billboardRowNum: Number(datasource.billboardRowNum) || 1
            }
        });
    })
...
vtron.comp.std.map.Cesium.prototype.invertMapColor = function(type=0) {
    ...
    if(type==0){
        ...
        vec3 sepia(vec3 base,vec3 sepia,float opacity){\n\
            vec3 sepia3;\n\
            float r = base.r;\n\
	        float g = base.g;\n\
	        float b = base.b;\n\
            sepia3.r = min(1.0, (r * (1.0 - (0.607 * opacity))) + (g * (0.769 * opacity)) + (b * (0.189 * opacity)));\n\
            sepia3.g = min(1.0, (r * 0.349 * opacity) + (g * (1.0 - (0.314 * opacity))) + (b * 0.168 * opacity));\n\
            sepia3.b = min(1.0, (r * 0.272 * opacity) + (g * 0.534 * opacity) + (b * (1.0 - (0.869 * opacity))));\n\
            return sepia3;\n\
            //return vec3(min(base.r*(1.0-opacity) + sepia.r*opacity,1.0),min(base.g*(1.0-opacity) + sepia.g*opacity,1.0),min(base.b*(1.0-opacity) + sepia.b*opacity,1.0));\n\
        }\n\
       \n\
       vec3 rgb2hsv(vec3 c) {\n\
        ...
        var mainstr = '\n\
            // Get map image\n\
           // vec4 texMapColor4 = computeDayColor(u_initialColor, clamp(v_textureCoordinates, 0.0, 1.0));\n\
            \n\
            //第1：    invert:100%\n\
            vec3 texMapColor3 = vec3(1.0 - color.rgb);\n\
            //第2：  深褐色 opacity(80%) 5C4033 //https://wenku.baidu.com/view/6dce28e24b7302768e9951e79b89680202d86b79.html \n\
            vec3 blendColor = vec3(93.0/255.0, 64.0/255.0, 51.0/255.0);\n\
            vec3 saturateColor3 =  sepia(texMapColor3,blendColor,0.8);\n\
            \n\
            vec3 hsvColor3 = rgb2hsv(saturateColor3);\n\
            //第四：hue-rotate(165);\n\
            hsvColor3.x += 165.0/360.0;\n\
            //第三：saturate(200%)\n\
            hsvColor3.y *= 2.0;\n\
            //第五：brightness(100%) 图像亮度不变化所以不加处理\n\
            \n\
            vec3 rgbColor3 = hsv2rgb(hsvColor3);\n\
            vec4 finalColor = vec4(rgbColor3, 1.0);\n\
            int invertType = 0;\n\
            \n\
        ';
        fragmentShader = baseFragmentShaderSource.sources[baseFragmentShaderSourceId].replace(/vec4 finalColor = color/g, mainstr);
 ...
 * fireTag,相应操作是否会发出事件
 */
vtron.comp.std.map.Cesium.prototype.changeState = function(name, fireTag = true, searchEventType = 'mapSearch_inside') {
    this.fireEventTag = fireTag;
    if(name == 'move'){
        this.selectingTag = false;
        if(this.optState == 'draw-line' || this.optState == 'draw-imageLine' ||this.optState == 'draw-buffer'){
            if(this.getEntityById(this.optState,this.optID+'.buffer') && this.getEntityById(this.optState,this.optID+'.buffer').holeNum>0){
                let length = this.getEntityById(this.optState,this.optID+'.buffer').holeNum;
                for(let i = 0;i<length;i++){
                    this.deleteElements(this.optState,this.optID+'.buffer.outline'+i);
                }
            }
            this.deleteElements(this.optState,this.optID);
            this.deleteElements(this.optState,this.optID+'.buffer.outline');
            this.hideElements('globeLayer','globeDeleteStyle');
        }
    ...
    if((this.optVertices && this.optVertices.length>0) && (this.optChooseResult == false)&&this.optState!='draw'){
        if(this.optState != name){
            ...
            this.deleteElements(this.optState,this.optID);
            //折线时把图标也要删掉
            if(this.optState == 'draw-line' || this.optState == 'draw-buffer'){
                this.hideElements('globeLayer','globeDeleteStyle');
            }
        }
    }
...
vtron.comp.std.map.Cesium.prototype.moveCenter = function(x, y, level, duration = 1, flightCompleteCallback) {
    ...
    var heading = Cesium.Math.toRadians(0.0),
        pitch = Cesium.Math.toRadians(this.pitch<0? this.pitch:-this.pitch),
        range = z;
...
vtron.comp.std.map.Cesium.prototype.zoom = function(level) {
    ...
    let zoomHeight = this._viewer.camera._positionCartographic.height - height;
    if(zoomHeight > 0){
        this._viewer.camera.zoomIn(zoomHeight);
    }else{
        this._viewer.camera.zoomOut(-zoomHeight);
    }
};
...
vtron.comp.std.map.Cesium.prototype.zoomIn = function() {
    ...
    var height = this.levelConvertToHeight(level);
    this._viewer.camera.zoomIn(this._viewer.camera._positionCartographic.height - height);
};
...
vtron.comp.std.map.Cesium.prototype.zoomOut = function() {
    var level = this.getLevel() - 1;
    var height = this.levelConvertToHeight(level);
    this._viewer.camera.zoomOut(height - this._viewer.camera._positionCartographic.height);
};
            ...
            if(data.value>1){
                ...
                entity = datasource.entities.add({
                    ...
                    billboard : {
                        ...
                        //distanceDisplayCondition : new Cesium.DistanceDisplayCondition(0, 20000000000),
                        //verticalOrigin : Cesium.VerticalOrigin.BOTTOM
                    },
                    label : {
 ...
 */
vtron.comp.std.map.Cesium.prototype.flytoRectangle = function(bbox, rate = 1,orientation={}){
    if(!this._viewer) return;
    if(this._viewer.trackedEntity){
        this.trackedCancel();
        return;
    ...
    if(orientation.heading!=undefined || orientation.pitch!=undefined || orientation.roll!=undefined){
        //计算中心
        let center = Cesium.Rectangle.center(Cesium.Rectangle.fromDegrees(bbox.west, bbox.south, bbox.east, bbox.north));
        this._viewer.camera.lookAt(Cesium.Cartographic.toCartesian(center), new Cesium.HeadingPitchRange(
            Cesium.Math.toRadians(orientation.heading==undefined? 0.0:orientation.heading),
            Cesium.Math.toRadians(-(orientation.pitch==undefined? 90.0:orientation.pitch)),
            Cesium.Cartesian3.distance(Cesium.Cartographic.toCartesian(center),this._viewer.camera.position) || 120000
        ))
    }else{
        this._viewer.camera.flyTo({
   ...
   dataInfo: [
       {
            id: 每个点的id，唯一
            text: 标注文本内容
            font: 标注文本字体，默认是'15pt sans-serif'
            fillColor: 标注文字的颜色,rgba或者16进制，默认白色
            outlineColor: 标注文字的轮廓颜色,rgba或者16进制,如果没有默认和fillColor一个色
            outlineWidth: 标注文字轮廓的宽度，单位像素，默认是1
            showBackground: 标注有无背景，默认无，透明，有就填rgba或者16进制
            backgroundColor： 标注文字背景颜色，rgba或者16进制，默认浅黄色
            position:{lon:xxx, lat:xxx, height: xxx},高度默认0
            imageUrl:图标路径
            imageScale: 图标放大的大小，默认是1
            offset: imageScale为1时，图标的大小，单位像素，默认25
       }
   ]
 */
vtron.comp.std.map.Cesium.prototype.addLabelElement = function(layerName,dataInfo){
    ...
    if(!(dataInfo instanceof Array)) dataInfo = [dataInfo];
    dataInfo.forEach((data) => {
        var tposition = Cesium.Cartesian3.fromDegrees(data.position.lon || 0, data.position.lat || 0, data.position.height||0);
        var entity = datasource.entities.getById(data.id);
        if(entity){
            //先只更新label的属性
            //entity.billboard.image = imageUrl;
            ...
            if(!data.outlineColor || !data.backgroundColor || !data.fillColor || !data.imageScale || !data.offset){
                data.fillColor = data.fillColor || 'rgba(255, 255, 255, 1.0)';
                data.outlineColor = data.outlineColor || data.fillColor;
                data.backgroundColor = data.backgroundColor || 'rgba(100,100,0,0.1)';
                data.imageScale = data.imageScale||1;
                data.offset = data.offset || 25;
            }
            entity = datasource.entities.add({
                id: data.id,
                name:layerName,
                ...
                billboard : {
                            image : data.imageUrl,
                            show : true, // default
                            //scale: this.imageScaleFactor * data.imageScale,
                            //scaleByDistance : new Cesium.NearFarScalar(1000, 0.5, 100000, 0.1),
                            height: this.imageScaleFactor *data.imageScale *data.offset,
                            width: this.imageScaleFactor *data.imageScale *data.offset,
                            verticalOrigin : Cesium.VerticalOrigin.BOTTOM,
                            //distanceDisplayCondition : new Cesium.DistanceDisplayCondition(0, 20000000000),
                },
                // point : {
                ...
                label : {
                    ...
                    verticalOrigin : Cesium.VerticalOrigin.BOTTOM,
                    horizontalOrigin : Cesium.HorizontalOrigin.CENTER,
                    pixelOffset : new Cesium.Cartesian2(0, -this.imageScaleFactor *data.imageScale *data.offset),
    ...
    dataInfo.forEach((data) => {
        if(data.id){
            ...
            if(!bexistEntity){//如果存在该entity，则不做处理
                ...
                //存储实际的时间
                var temptTime = data.path[0]["gps_time"].split(" ");
                var realStartTime = Cesium.JulianDate.fromIso8601(temptTime[0]+"T"+temptTime[1]);
                realStartTime = Cesium.JulianDate.addHours(realStartTime, 8, new Cesium.JulianDate());
                this.pathInfo.realStartTime = realStartTime;
                this.pathInfo.startTime = start;
                this.pathInfo.playTime = playTime;
                this.pathInfo.realPlayTime = data.path[data.path.length-1].time/1000;
                ...
                for (var i = 0; i < data.path.length; i++) {
                    ...
                    if(data.path[i].event){
                        if(data.path[i].isend!=undefined && data.path[i].isend){
                            this.pathLog.push({'id':data.id,'time':time,'event':data.path[i].event,'isend':data.path[i].isend,'x':data.path[i].lon, 'y':data.path[i].lat});
                        }else{
                            this.pathLog.push({'id':data.id,'time':time,'event':data.path[i].event,'x':data.path[i].lon, 'y':data.path[i].lat});
                        }
                    }else{
                        if(i == data.path.length - 1){
                            this.pathLog.push({'id':data.id,'order': i, 'time': time, 'event':'sendWholeTime','x':data.path[i].lon, 'y':data.path[i].lat,'tag':'end'});
                        }else{
                            //整个播放没次走都要抛出事件的情况
                            this.pathLog.push({'id':data.id,'order': i, 'time': time, 'event':'sendWholeTime','x':data.path[i].lon, 'y':data.path[i].lat});
                        }
                    }
                }
                //计算路径宽度
                ...
                if(data.image.indexOf("gltf")>0){
                    ...
                    entity = datasource.entities.add({
                        ...
                        path : {
                            ...
                            // }),
                            // material :new Cesium.ImageMaterialProperty({
                            //     image : '/vmap2/images/路线3.png',
                            //     transparent : true,
                            //     repeat: new Cesium.Cartesian2(20.0,1.0)
                            // }),
                            material :new Cesium.StripeMaterialProperty({
                                oddColor: Cesium.Color.BLACK,
                                evenColor: Cesium.Color.fromCssColorString('rgba(0,255,255,0.5)'),
                                transparent : true,
                                offset:3,
                                repeat: 100
                            }),
                            width : pathWidth/2 || 10,
                        },
                    });
                    //还没跑完的path
                    ...
                    var entityPath = datasource.entities.add({
                        ...
                        path : {
                            ...
                            //     color : Cesium.Color.fromCssColorString(pathColor) || Cesium.Color.PALEGOLDENROD,
                            // }),
                            material: new Cesium.PolylineTrailLinkMaterialProperty(Cesium.Color.RED, 2000)//动态材质
                        }
                    });
                }
    ...
    if(this._viewer){
        ...
        if(datasource.pathTrackingLayer){ 
            ...
            //     this._viewer.clockViewModel.shouldAnimate = true;
                this._viewer.clock.currentTime = this.pathTracking_startTime;
                if(this.pathLog.length>0){
                    this.pathLog[this.pathLog.length-1].tag = 'end';
                }
            } 
        }
    ...
    if(this._viewer){
        ...
        if(datasource.pathTrackingLayer){ 
            ...
            dataInfo.forEach((data) => {
                ...
                var joinLineEntity = datasource.entities.getById(data+'-join_line');
                if(joinLineEntity){
                    joinLineEntity.polyline.position = [];
    ...
    if(!datasource.pathTrackingLayer){
        ...
        if(fids.length!=0){
            fids.forEach(id => {
                ...
                if(entity){
                    if(entity.polygon){
                        if(entity.polygon.show && entity.polygon.show.getValue()==true){
                            entities.push(entity);
                        }else{
                            var outlineEntity = datasource.entities.getById(id+'.outline');
                            var bufferEntity = datasource.entities.getById(id+'.buffer');
                            if(outlineEntity){
                                entities.push(outlineEntity);
                            }
                            if(bufferEntity){
                                entities.push(bufferEntity);
                            }
                        }
                    }else if(entity.polyline){
                        if(entity.polyline.show && entity.polyline.show.getValue() == true){
                            entities.push(entity);
                        }else{
                            var outlineEntity = datasource.entities.getById(id+'.outline');
                            var bufferEntity = datasource.entities.getById(id+'.buffer');
