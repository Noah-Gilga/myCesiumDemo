                    ...
                    if(entity) {
                        if(typeof(entity)=="object"){
                            ...
                            if(pickObject.primitive.geoType=='simpleLine'){
                                ...
                                myThis.fire("lineDoubleClickEvent", data);
                                return;
                            }else if(pickObject.primitive.geoType=='billboardTest'){
                                let nowPosition = myThis.getCartographicFromScreenCoord(position.x,position.y);
                                var data = {
                                    id: pickObject.primitive.id,
                                    _id: "_" + pickObject.primitive.id,
                                    x: nowPosition.lon,
                                    y: nowPosition.lat,
                                    info: pickObject.primitive.otherProperty,
                                    layerName: 'alert',
                                    position: pickObject.primitive._actualPosition
                                }
                                myThis.fire("layerDoubleClickEvent", data);
                                return;
                            }
...
vtron.comp.std.map.Cesium.prototype.addBillboardPointElement = function(layerName, dataInfo, imageUrl, scale=1.0){
    if(!this._viewer) return;
    var billboardCollection = this.getPrimitiveCollectionByName(layerName);
    if(!billboardCollection){
        billboardCollection = new Cesium.BillboardCollection();
        this._viewer.scene.primitives.add(billboardCollection);
        billboardCollection.geoType = 'billboardTest';
        billboardCollection.name = layerName;
    }
    if(!dataInfo) return;
    if(!(dataInfo instanceof Array)) dataInfo = [dataInfo];
    var instance = this;
    (dataInfo.length>0) && dataInfo.forEach((data) => {
        var billboard = billboardCollection.add({
            id: data.id,
            show : true,
            image : data.imageUrl||imageUrl,
            position: Cesium.Cartesian3.fromDegrees(Number(data.x || 0.0), Number(data.y || 0.0), 0.0),
            scale : this.imageScaleFactor * scale,
            verticalOrigin : Cesium.VerticalOrigin.BOTTOM,
            distanceDisplayCondition: new Cesium.DistanceDisplayCondition(0.0, 2000000.0),
            scaleByDistance : new Cesium.NearFarScalar(2000, 1.0, 20000, 0.3),
        })
        billboard.geoType = 'billboardTest';
        billboard.otherProperty = data.info;
        billboard.x = data.x || 0.0;
        billboard.y = data.y || 0.0;
    })
}
...
vtron.comp.std.map.Cesium.prototype.addPointElement = function(layerName, dataInfo, imageUrl, scale = 1, height = 0, bShowLayer = true, supportSearch = false, isScaleByDistance = true) {
    if(!this._viewer) return;
    if(layerName =='alert'){
        this.addBillboardPointElement(layerName, dataInfo, imageUrl, scale);
        return;
    }
...
vtron.comp.std.map.Cesium.prototype.pointsChangeStyle = function(layerName, dataInfo, imageUrl, scale = 1) {
    ...
    if(!(dataInfo instanceof Array)) dataInfo = [dataInfo];
    if(datasource.geoType == 'billboardTest'){
        dataInfo.length>0 && dataInfo.forEach((data) => {
            var entity = null;
            datasource._billboards.forEach((billboard)=>{
                if(billboard.id == data.id){
                    entity = billboard;
                }
            })
            if(entity){
                if(data.imageUrl || imageUrl){
                    entity.image = data.imageUrl||imageUrl;
                    entity.imageSrc = data.imageUrl||imageUrl;
                }
                if(data.imageUrlPicked){
                    entity.imagePicked = data.imageUrlPicked;
                }
                entity.scale = this.imageScaleFactor*scale;
            }
        });
    }else{
        dataInfo.forEach((data) => {
...
vtron.comp.std.map.Cesium.prototype.getDataSourceByName = function(layerName) {
    if(layerName == 'alert'){
        return this.getPrimitiveCollectionByName(layerName);
    }
...
vtron.comp.std.map.Cesium.prototype.getBillboardById = function(layerName, id) {
    if(!id) return;
    var billboardCollection = this.getPrimitiveCollectionByName(layerName);
    if(billboardCollection){
        (billboardCollection._billboards.length>0) && billboardCollection._billboards.forEach((billboard)=>{
            if(billboard.id == id){
                return billboard;
            }
        })
...
vtron.comp.std.map.Cesium.prototype.getEntityById = function(layerName, id) {
    if(layerName =='alert'){
        this.getBillboardById(layerName, id)
    }
    var datasource = this.getDataSourceByName(layerName);
    var result = null;
    if(datasource) {
...
vtron.comp.std.map.Cesium.prototype.showElements = function(layerName, fids) {
    if(layerName == 'alert'){
        this.showBillboardElements(layerName, fids);
        return;
    }
    var datasource = this.getDataSourceByName(layerName);
...
vtron.comp.std.map.Cesium.prototype.hideBillboardElements = function(layerName, fids) {
    var billboardCollection = this.getPrimitiveCollectionByName(layerName);
    if(!fids) return;
    if(!(fids instanceof Array)) fids = [fids];
    if(billboardCollection && billboardCollection._billboards){
        if(fids.length == 0){
            ...
            for(var i = 0 ;i<fids.length;i++){
                for(var j =0;j<billboardCollection._billboards.length;j++){
                    if(billboardCollection._billboards[j].id ==fids[i]){
                        billboardCollection._billboards[j].show = false;
                        break;
                    }
                }
            }
...
vtron.comp.std.map.Cesium.prototype.showBillboardElements = function(layerName, fids) {
    var billboardCollection = this.getPrimitiveCollectionByName(layerName);
    if(!fids) return;
    if(!(fids instanceof Array)) fids = [fids];
    if(billboardCollection && billboardCollection._billboards){
        if(fids.length == 0){
            ...
            for(var i = 0 ;i<fids.length;i++){
                for(var j =0;j<billboardCollection._billboards.length;j++){
                    if(billboardCollection._billboards[j].id ==fids[i]){
                        billboardCollection._billboards[j].show = true;
                        break;
                    }
                }
            }
...
vtron.comp.std.map.Cesium.prototype.deleteBillboardElements = function(layerName, fids) {
    var billboardCollection = this.getPrimitiveCollectionByName(layerName);
    if(!fids) return;
    if(!(fids instanceof Array)) fids = [fids];
    if(billboardCollection && billboardCollection._billboards){
        if(fids.length == 0){
            this._viewer.scene.primitives.remove(billboardCollection)
        }else{
            for(var i = 0 ;i<fids.length;i++){
                for(var j =0;j<billboardCollection._billboards.length;j++){
                    if(billboardCollection._billboards[j] && billboardCollection._billboards[j].id ==fids[i]){
                        billboardCollection.remove(billboardCollection._billboards[j])
                        break;
                    }
                }
            }
...
vtron.comp.std.map.Cesium.prototype.hideElements = function(layerName, fids) {
    if(layerName == 'alert'){
        this.hideBillboardElements(layerName, fids);
        return;
    }
    var datasource = this.getDataSourceByName(layerName);
    var primitives = undefined;
...
vtron.comp.std.map.Cesium.prototype.deleteElements = function(layerName, fids=[]) {
    if(layerName == 'alert'){
        this.deleteBillboardElements(layerName, fids);
        return;
    }
    var datasource = this.getDataSourceByName(layerName) || this.getPrimitiveCollectionByName(layerName);
    if(!fids){
...
vtron.comp.std.map.Cesium.prototype.layerShow = function(layerName, bPrimitive = false,ballElementShow=false) {
    if(layerName == 'alert'){
        this.showBillboardElements(layerName, []);
    }
    var layer = null;
    //如果layerName是未定义或者空，没有意义返回
    if(layerName==null||!layerName){
...
vtron.comp.std.map.Cesium.prototype.layerHide = function(layerName, bPrimitive = false,ballElementHide=false) {
    if(layerName == 'alert'){
        this.hideBillboardElements(layerName, []);
    }
    var layer = null;
    //如果layerName是未定义或者空，没有意义返回
    if(layerName==null||!layerName){
...
vtron.comp.std.map.Cesium.prototype.layerDelete = function(layerName, dataInfo, bPrimitive = false) {
    if(layerName == 'alert'){
        this.deleteBillboardElements(layerName, []);
    }
    if(!layerName) {
...
vtron.comp.std.map.Cesium.prototype.searchByTurf = function(layerNameArray, optID, optState, vertices, lineWidth, radius = 0, otherProperty,bCmd, stopFire) {
    ...
    layerNameArray.forEach((layerName) => {
        ...
        if(datasource) {
            switch(optState) {
                case 'choose-line':
                    ...
                    if(datasource.geoType == 'billboardTest'){
                        datasource._billboards.forEach((entity) => {
                            if(entity.x && entity.y){
                                var pt = turf.point([entity.x, entity.y]);
                                var bChoose = turf.booleanPointInPolygon(pt, polyBuffer);
                                if(bChoose) {
        ...
        layerResult.forEach((entity) => {
            ...
            //lonlat = {lon:,lat:}
            var lonlat = null;
            if(entity.geoType == 'billboardTest'){
                lonlat = this.getCartographicFromCartesian(entity._actualPosition.x, entity._actualPosition.y, entity._actualPosition.z);
            }else{
                lonlat = this.getCartographicFromCartesian(entity.position._value.x, entity.position._value.y, entity.position._value.z);
            }
            if(!entity.otherProperty){
                entity.otherProperty = {};
            }
